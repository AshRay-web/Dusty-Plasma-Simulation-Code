%Prepared By Mr. Roshan Chalise and Dr. Raju Khanal at 50/09/2012
...
% our 1d3v plasma-sheath simulation.
% This proceeds as following:
% 1) Provide the name of your input file, where all required input
...
for rphifN=1:length(phifNi)
   phifN=phifNi(rphifN);
	Ce=1+erf(sqrt(-phifN));
	De=exp(phifN);
	TfeN=1/abs(3 - sqrt(-4*phifN/pi)*De/Ce - 2/pi*(De/Ce)^2);
...
Ri=(sqrt(pi) + De/Ce/sqrt(-phifN))/TfeN *(1+ (nd/niL)*Qd0);
...
for r=1:length(TcLiNi)
   ...
   Di=exp(-TcLiN^2);
   TfiN=TpsiN/abs(3 - 2*TcLiN*Di/Ci/sqrt(pi) - 2*(Di/Ci)^2/pi);
...
Ae=AeN*nps*(Me/(2^.33)/pi/kB/Tpse)^1.5;
neL=0.5*Ae*vtfe^3*(pi)^1.5;
phi0=phifN*kB*Tfe/e;
% ion dimensional parameters:
Tfi=TfiN*Tpse;
vtfi=sqrt(2*kB*Tfi/Mi);
Ai=AiN*nps*(Mi/(2^.33)/pi/kB/Tpse)^1.5;
niL=nps;
