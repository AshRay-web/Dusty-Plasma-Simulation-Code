...
for T=1:ntra                                                      % Calculate for 1-ntra
    ...
    Vym = -2.3045e4;                                                   % The value of y-component velocity
    Vzm = -2.3045e4;                                                   % The value of z-component velocity
...
for j=1:nx;
   ...
   for T=1:ntra-1
       j=1:nx;
      Ei(j)=Ei(j)+(0.5*0.5*Mi*(V_xi(T+1).^2+V_xi(T).^2)*(Df(T)+Df(T+1))*abs(V_xi(T+1)-V_xi(T)))./(ni(j)+0.5*(Df(T)+Df(T+1))*abs(V_xi(T+1)-V_xi(T)));
      Eti(j)=Ei(j)/e;
      Ewi=Eti(1);
   end
   clear  V_xi T
