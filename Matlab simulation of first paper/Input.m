...
% To study the sheath structure for an oblique magnetic field:
    Angle =input('Put  the value of angle made by an oblique magnetic field along x-axis :');
    Theta=(Angle*pi/180);                               % In terms of radian

% Basic parameters:
    B0=-100e-3;                                          % Applied magnetic field(Tesla)
    vy=-2.3045e4;                                          % Value of Y-component velocity
    vz=-2.3045e4;                                           % Value of Z-component
...
% presheath parameters:
    ...
    Tpse=10*11604.9;                                  % Electron temperature at preseath side(k)
    Tpsi=0.08*Tpse;                                      % Ion temperature at preseath side(k)
    nps=1e18;                                           % Plasma density at preseath side(1/m^3)
    cpsi=sqrt(kB*(Tpse+gammai*Tpsi)/Mi);                % Ion acoustic velocity at presheath side (m/s)
    upsi=-cpsi;                                         % Average fluid velocity along x-axis(sheath edge singularity condition)(m/s)
...
% numerical input parameters:
    ntra=300;                                           % Total number of trajectories at sheath entarce between vicL & vimaxL
    phi0x='phi0*(1-X/L)';                               % Potential at X-position iss obtained by interpolation
    nx=41;                                              % Total number  of x-grids
    dt=1e-10;                                           % Time step size(s)
    deltaphi=1e-7;                                      % Desired accuracy in the iteration((phi2-phi1)>deltaphi)(volt)
    w=.08;                                              % Relaxitation parameter(relating to relaxitation scheme)
    nv=400;                                             % Total number of velocity grids


   phifNi=linspace(-5,-0.001,1001);                    % Initial guess (always negative)
   Tau_min=-5.8877254338;                              % Lower limit of variable 'Tau'. useless to keep below: -5.8877254338, since f -> 0.


   TcLiNi=linspace(-5.88,0,90);                        % initial guess	%upper limit for TpsiN=.1 -> 2.2373356648402
