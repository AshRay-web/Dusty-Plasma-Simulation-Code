...
for  j=2:nx-1                                                % Selecting an inner jth x-grid point from 2 to nx-1.
 ...
 DFAiondensity    % Calculate the ion density using DFA
     ...
     nin=ni/niL;
     [Qd_array, Qd0,phi_d] = oml_poisson_solver(ne, ni);                                                        % Normalized density of ion
    % neutrality-safe rho
     rho = Ze*(ni - nps) - e*(ne - nps) + n_dust*(Qd_array - Qd0);                                        %Adding OML rhod_single for single ni,ne initial using oml_solverin
...
DFAiondensity                                                % Calculate the new ion density using DFA
      ...
      % local dust charge at each gridpoint
      % Get local dust charge and reference charge
      [Qd_array, Qd0,phi_d] = oml_poisson_solver(ne, ni);
      ...
      % neutrality-safe rho
      rho = Ze*(ni - nps) - e*(ne - nps) + n_dust*(Qd_array - Qd0);                                      % Charge density distribution for new potential profile
      poissionsolver
     % Solving the poission equation for new potential
     plot(X/DLe,phi_d,"b")
 end

      plot(X/DLe,phi_d,'r')                                      % plot the graph between normalized distance and potential
      ne=neL_*exp(e*phi/kB/Tfe).*(1+erf(sqrt(e*(phi-phi(1))/kB/Tfe))); % Electron density distribution at grid point
      ni=niL*ni/max(ni); ne=niL*ne/max(ne);                    % Check the density goes out of the boundary region
      nin=ni/max(ni);                                          % Normalised ion density
      nen=ne/max(ne);       % Normalised electron density
