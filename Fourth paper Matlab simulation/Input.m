...
% ---------- DUST PARAMETERS (add these) ----------
rd   = 1e-6;        % grain radius in micrometers (um). Example: 1.0 um
n_dust   = 1e11;      % dust number density [m^-3] (uniform)
rho_mat = 1.925e4;    % material density [kg/m^3], tungsten ~ 1.925e4.
% -------------------------------------------------
...
% presheath parameters:
    ...
    ev=11604.9;
    K=1e3;
    Tpse=10*ev;                                  % Electron temperature at preseath side(k)
...
%sigma = Tpsi/Tpse;
%M_bohm = sqrt(1 + 3*sigma);
%assert(M >= M_bohm, 'Chosen M < warm Bohm threshold; increase M or lower Ti/Te.');
